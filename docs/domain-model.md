...
Temporal Worker yet — a live Worker is a later AAMP-1 step — matching
...
- **~~No live migration has been applied.~~ Resolved by AAMP-1 step 1
  (2026-07-26).** Docker is now available, and the initial migration
  `packages/database/prisma/migrations/20260726053508_init/` was generated by
  `prisma migrate dev` against a live PostgreSQL 16 instance and applied. It
  covers every model in `schema.prisma` — including every model added since
  M10 — with no drift (`prisma migrate diff` reports `No difference detected.`)
  and applies cleanly to an empty database. Operational procedure:
  `docs/runbooks/database-migrations.md`. What remains unproven here is
  everything _downstream_ of the schema: no application process has yet been
  run against live Postgres (`apps/api` and the repository test suite still run
  against the in-memory store), and Temporal, MinIO and real authentication are
  later AAMP-1 steps.
...
execute-specialist-agent-activity.ts`) but is not one of this document's
  ...
  exists yet (see that section's note in architecture.md §6) — the
  `WorkflowRun` table is scoped to AAMP-1. Like every other model in this
  schema, it is now covered by the initial migration (see the resolved
  limitation above).
